...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- Yimina & Noélie BOTTERO
       Exercice 5.2
       Feuille de style xslt dont le résultat est un document HTML contenant le texte et
       les mots dont le lemme est « aristocrate » ou « patriote » sont surlignés -->
            ...
            <body>
                <xsl:for-each-group select="//w" group-ending-with="w[forme='.']"><p style="text-align:justify">
                    <xsl:for-each select="current-group()">
                        <xsl:choose>
                            <xsl:when test="contains(lemme/text(), 'patriote') or (contains(lemme/text(), 'aristocrate'))">
                                <xsl:call-template name="color">
                                    <xsl:with-param name="word" select="current()"/>
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:when test="matches(lemme/text(), '(patriot(es?)|aristocrat(es?)*\.)')">
                                <mark><xsl:value-of select="contains(forme/text(), '([a-zA-Z]*\.)')"/></mark>
                            </xsl:when>
